...
public class ChatService {
    ...
    public String handleChat(ChatRequestDTO request) {
        ...
        JD jd = jdRepository.findByDriveId(request.getDriveId())
                .orElseThrow(() -> new RuntimeException("JD not found for drive"));
        String jdText = jd.getJdText();
    ...
    private String buildPrompt(String jdText, List<String> skills, String question) {
        return "You are an AI chatbot helping a user prepare for a company placement drive.\n\n"
                + "The user's skills are:\n" + String.join(", ", skills) + "\n\n"
                + "The Job Description (JD) for the company drive is:\n" + jdText + "\n\n"
                + "Answer the following question concisely and to the point. Do not include unnecessary lines, symbols, or formatting. "
                + "Keep the explanation simple and easy to understand. Provide precise answers, as longer responses will increase API token usage.\n"
                + "Question: \"" + question + "\"";
    }
